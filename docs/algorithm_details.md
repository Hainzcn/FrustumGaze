...
## 4. 手部姿态估计与多源融合 (Hand Pose Estimation & Multi-source Fusion)

手部追踪在本项目中经历了从单一 PnP 解算到多源数据融合的演进。为了解决手掌平面二义性、遮挡导致的 PnP 不稳定以及快速运动下的抖动问题，我们构建了一套复杂的评估与融合体系。

### 4.1 综合置信度评估 (Comprehensive Confidence Score, $q$)

在进行姿态解算前，首先对当前帧的手部数据质量进行量化评估，输出一个综合置信度 $q \in [0, 1]$。该分数由三个子指标构成：

1.  **关键点置信度 ($q_{lm}$)**:
    *   结合 MediaPipe 输出的手部存在置信度 (Handedness Score)。
    *   **手掌开放度 (Openness)**: 通过计算手指长度与手掌长度的比值，判断手是否处于握拳状态。握拳时关键点挤压在一起，PnP 解算极不可靠，因此会大幅降低 $q$ 值。

2.  **几何一致性 ($q_{geo}$)**:
    *   **形状因子 (Shape Factor)**: 计算手掌关键点围成的多边形面积与周长平方的比值 ( $\text{Area} / \text{Perimeter}^2$ )。该指标对深度变化不敏感，能有效衡量手掌是否发生形变或检测错误。当形状因子偏离正常手掌范围时，降低置信度。

3.  **重投影误差 ($q_{rep}$)**:
    *   将解算出的 3D 姿态反投影回 2D 图像，计算与实际观测点的像素距离误差 (RMSE)。误差越小，PnP 解越可信。

最终置信度： $q = q_{lm} \cdot q_{geo} \cdot q_{rep}$

### 4.2 多源姿态估计通道 (Multi-source Estimation Channels)

为了避免单一依赖 PnP 造成的不稳定性，我们建立了三条并行的法向量估计通道：

*   **通道 A (PnP 解算)**:
    *   方法：利用 `cv2.solvePnP` 解算 3D-2D 对应关系。
    *   特点：精度最高，但对噪声和遮挡极其敏感。
    *   权重 $w_A$：与置信度 $q$ 正相关。

*   **通道 B (SVD 平面拟合)**:
    *   方法：对 3D 空间中的手掌关键点（腕、指根）进行奇异值分解 (SVD)，求取最小二乘拟合平面的法向量。
    *   特点：对单点噪声鲁棒，计算稳定，不易受 PnP 奇异值影响。
    *   权重 $w_B$ ：固定基础权重。

*   **通道 C (运动预测)**:
    *   方法：利用 `AdaptiveEKF` 的预测步骤，根据上一帧的状态推演当前帧的法向量。
    *   特点：在完全遮挡或检测失效时提供惯性维持。
    *   权重 $w_C$：与置信度 $q$ 负相关 (即 $1-q$)。

### 4.3 加权融合策略 (Weighted Fusion)

最终的手掌法向量 $\vec{n}_{final}$ 是三个通道的加权归一化和：

$$\vec{n}_{raw} = w_A \cdot \vec{n}_A + w_B \cdot \vec{n}_B + w_C \cdot \vec{n}_C$$

$$\vec{n}_{final} = \frac{\vec{n}_{raw}}{\| \vec{n}_{raw} \|}$$

该策略确保了在数据质量好时（$q \to 1$）优先使用高精度的 PnP 结果；在数据质量差或握拳时（$q \to 0$），平滑过渡到 SVD 拟合和运动预测结果，彻底消除了“鬼影”和跳变。

### 4.4 自适应扩展卡尔曼滤波 (Adaptive EKF)

融合后的法向量仍需进行时域平滑。我们设计了一个针对球坐标系的自适应扩展卡尔曼滤波器。

*   **状态向量**: $[\theta, \phi, \dot{\theta}, \dot{\phi}]^T$ 。使用球坐标系（极角 $\theta$ ，方位角 $\phi$ ）代替直角坐标，避免了法向量归一化的非线性约束问题。
*   **动态噪声协方差 ( $R$ )**:
    观测噪声矩阵 $R$ 根据置信度 $q$ 动态调整：
    $$R = R_{base} / q^2$$
    *   当 $q$ 高时，$R$ 变小，滤波器快速跟随观测值。
    *   当 $q$ 低时，$R$ 变大，滤波器更多依赖预测模型（惯性滑行）。
*   **极点奇异性处理**:
    当法向量接近 Z 轴（ $\theta \approx 0$ 或 $\pi$ ）时，方位角 $\phi$ 会出现万向节死锁 (Gimbal Lock) 现象。滤波器会自动检测此区域并大幅增加 $\phi$ 的观测噪声，防止数值发散。
