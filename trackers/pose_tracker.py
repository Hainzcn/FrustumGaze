...
class PoseProcessorProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        # 2. 初始化 MediaPipe Pose (Tasks API)
        try:
            ...
            print(f"PoseProcessorProcess: Failed to init MediaPipe: {e}")
            return
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if processed_rgb_full is not None:
                    ...
                    pose_result = pose_detector.detect_for_video(mp_image_pose, timestamp_ms)
                    
                    if pose_result.pose_landmarks:
                        # Extract Shoulders, Elbows (Removed Wrists as requested)
                        ...
                        # 我们只需要肩和肘用于绘制和连接
                        # 但为了方便 Visualizer 匹配，我们保留原来的顺序或结构
                        # 之前的 indices = [11, 12, 13, 14, 15, 16]
                        # 用户要求: "不绘制人体追踪中的手腕节点"
                        # 所以我们只传肩和肘: 11, 12, 13, 14
                        indices = [11, 12, 13, 14]
